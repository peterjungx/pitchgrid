<script lang='ts'>
    ...
    import { Grid, Checkbox, Space, NumberInput,ActionIcon, Group, Button } from '@svelteuidev/core';
    import {  Play, Stop } from 'radix-icons-svelte';
    import Slider from '$lib/components/Slider.svelte';

    import { angle, calc_scale } from './lattice_math';
    import type {system, MappedScaleDisplayData, nodecoord} from './lattice_math';
    ...
    let window_width = 12;
    let window_offset = 4;
    let y_scale_factor = 12;
    let x_scale_factor = 7;
...
<Grid cols={16}>
    ...
    <Grid.Col span={2}>
        8ave width {octave.toFixed(3)}
        <Slider bind:value={octave} min={0.2} max={4.5} step={0.0001} />
        pitch angle {G_s.toFixed(3)}
        <Slider bind:value={G_s} bind:min={cpa_slider_min} bind:max={cpa_slider_max} step={0.001} />
        <Space h={5}/>
    </Grid.Col>
    <Grid.Col span={2}>
        generator {G.toFixed(3)}
        <Slider bind:value={G} min={0} max={1} step={0.0001}/>
    </Grid.Col>
    <Grid.Col span={2}>
        window width {window_width}
        <Slider bind:value={window_width} min={2} max={36} step={1} />
        window offset {Math.round(window_offset)}
        <Slider bind:value={window_offset} min={1} bind:max={window_width} step={0.1} />
    </Grid.Col>
    <Grid.Col span={2}>
...
<div class="container" bind:clientWidth={w} bind:clientHeight={h}>
    <svg width="100%" height="100%" viewBox="{-centerX} {-centerY} {w} {h}" xmlns="http://www.w3.org/2000/svg">
            ...
            <WindowLattice 
                ...
                edge_length={display_data.edge_length}
                color="green"
                s_target={display_data.s_target}
                dual={display_data.dual}
                affine_t={affine_t}
                tuning_data={tuning_data}
